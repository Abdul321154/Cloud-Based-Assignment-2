...
export default function PhonemeWordleBuilder() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchActivities("wordle");
  ...
  function generateFromFields() {
    const phonemes = phonemeWord.trim() ? phonemeWord.trim().split(/\s+/) : [];
    if (phonemes.length === 0) {
      setNotice("Enter a phoneme word before generating.");
      return;
    }
    setNotice(null);
    setMarkup(
  ...
  function loadSelected(id: string) {
    ...
    setPhonemeWord(config.targetPhonemes.join(" "));
    setEnglishWord(config.targetEnglish ?? "");
    setNumGuesses(config.numGuesses ?? 6);
    setShowHints(Boolean(config.showHints));
    setNotice(null);
    // Generate preview directly from stored database data.
    setMarkup(createWordleMarkup(config));
  }

  async function saveCurrent() {
    const phonemes = phonemeWord.trim() ? phonemeWord.trim().split(/\s+/) : [];
    if (phonemes.length === 0) {
      setNotice("Enter a phoneme word before saving.");
      return;
    }
    setNotice(null);
    try {
      const created = await createActivity({
        name: `Wordle — ${englishWord || "untitled"}`,
        activityType: "wordle",
        difficulty: "medium",
        showHints,
        numGuesses,
        rows: null,
        cols: null,
        words: [{ english: englishWord || undefined, phonemes: phonemes.map((symbol) => ({ symbol })) }],
      });
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col text-start space-y-4">
          ...
          <textarea
            ...
            placeholder="Enter phonemes separated by spaces (e.g. tʃ æ t)"
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {markup && (
              <button
                ...
                onClick={() =>
                  triggerMarkupExport(
                    `phoneme-wordle-${STUDENT_STAMP.downloadKey}.html`,
                    markup,
                  )
                }
